...
class Pagination
{
    ...
    public function get()
    {
        ...
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            ...
            if ($page == $this->current_page) {
                $links .= '<li class="page-nav__item"><a class="page-nav__item__link--active">' . $page . '</a></li>';
            } else {
        ...
        if (!is_null($links)) {
            ...
            # Создаём ссылку "На первую"
                $links = $this->generateHtml(1, '<i class="fa fa-angle-double-left"></i>') . $links;

            # Если текущая страница не первая
            if ($this->current_page < $this->amount)
            # Создаём ссылку "На последнюю"
                $links .= $this->generateHtml($this->amount, '<i class="fa fa-angle-double-right"></i>');
        }
    ...
    private function generateHtml($page, $text = null)
    {
        ...
        return
                '<li class="page-nav__item"><a href="' . $currentURI . $this->index . $page . '" class="page-nav__item__link">' . $text . '</a></li>';
    }
